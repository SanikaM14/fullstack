...
const MemoryCard = ({ memory, onDelete }) => {
  ...
  const toggleExpand = (e) => {
    ...
    if (willExpand) {
      let fact = "Every memory has a story.";
      if (memory.dominantColors && memory.dominantColors.length > 0) {
        fact = `Dominant color: ${memory.dominantColors[0]}`;
      }
      if (memory.year !== "Unknown") {
        const diff = new Date().getFullYear() - parseInt(memory.year);
        if (diff > 0) fact = `This memory is ${diff} years old!`;
      }
      triggerState('memoryCompanion', { text: fact });
    } else {
      triggerState('idle');
    }
  };
  ...
  return (
    ...
    >
      ...
      {isExpanded && (
        <div className="card-details" style={{ position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', top: '10px', right: '10px' }}>
            <Mascot width="100px" height="100px" isMini={true} />
          </div>
        </div>
      )}
